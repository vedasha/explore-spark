...
object LDAExample {
  ...
  def preprocess(
    ...
    val dfInitial = spark.read.json("data/news.json.gz").filter($"language" === "en").select("text")
    val initialrdd = dfInitial.rdd

    print(s"total number of records: ${dfInitial.count()}")
    ...
    val rdd = initialrdd.mapPartitions { partition =>
      ...
      partition.map { row: Row =>
        ...
        getLemmaText(value, morphology)
      }
    }.map(filterSpecialCharacters)
    rdd.cache()
    ...
    val documents = transformed.select("features").rdd.map {
